...
def encrypt_blob(blob, public_key, filename):
    ...
    end_loop = False
    encrypted = bytearray()
    outputFile = "(EncryptedRSA)"+filename
...
unencrypted_file = Path('test.txt')
encrypted_file = unencrypted_file.with_suffix('.dat')

encrypted_msg = encrypt_blob(
    unencrypted_file.read_bytes(), public_key.read_bytes(), 'test.txt')
